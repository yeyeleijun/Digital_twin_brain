...
#if DEBUG
template<typename T, typename T2>
void test_V_membranes(NodeInfo<T, T2>& node)
{
	if(node.block_->block_type_ == BrainBlock<T, T2>::BLOCK_TYPE::BLOCK_TYPE_NORMAL)
	{
		vector<T> v_membranes_gpu(node.block_->get_total_neurons());
		const T* v_membranes_cpu = node.block_->get_V_membranes_cpu(false);
		HIP_CHECK(hipMemcpy(v_membranes_gpu.data(), node.block_->get_V_membranes_gpu(), v_membranes_gpu.size() * sizeof(T), hipMemcpyDeviceToHost));
		node.block_->update_V_membrane_cpu();
		float max_err = FLT_MIN;
		unsigned int max_idx;
	
		for(unsigned int j = 0; j < v_membranes_gpu.size(); j++)
		{
			float diff = fabsf(v_membranes_cpu[j] - v_membranes_gpu[j]) / (fabsf(v_membranes_cpu[j]) + FLT_MIN);
			if(diff > max_err)
			{
				max_err = diff;
				max_idx = j;
		...
		{
			std::stringstream ss;
			ss << "the max relative error of V membrane at " << max_idx << ": " << max_err << endl;
			ss << "cpu result: " << v_membranes_cpu[max_idx] << " vs. gpu result: " << v_membranes_gpu[max_idx] << endl;
			LOG_INFO << ss.str();
		}
		else
		{
			LOG_INFO << "the max relative error of V membrane: 0" << endl;
		}
	}
}

template<typename T, typename T2>
void test_F_actives(NodeInfo<T, T2>& node)
{
	if(node.block_->block_type_ == BrainBlock<T, T2>::BLOCK_TYPE::BLOCK_TYPE_NORMAL)
	{
		vector<unsigned char> f_actives_gpu(node.block_->get_total_neurons());
		node.block_->update_F_active_gpu(0.f, 1.f, true);
		HIP_CHECK(hipMemcpy(f_actives_gpu.data(), node.block_->get_F_actives_gpu(), f_actives_gpu.size(), hipMemcpyDeviceToHost));
		node.block_->update_F_active_cpu();
		const unsigned char* f_actives_cpu = node.block_->get_F_actives_cpu(false);

		bool is_same = true;
		vector<unsigned int> diffs;
		for(unsigned int j = 0; j < f_actives_gpu.size(); j++)
		{
			if(f_actives_gpu[j] != f_actives_cpu[j])
			{
				diffs.push_back(j);
				is_same = false;
			}
		...
		if(is_same)
		{
			LOG_INFO << "both arrays of spike are same." << endl;
		}
		else
		{
			std::stringstream ss;
			ss << "both arrays of spike have " << diffs.size() << " different values:" << endl;
...
template<typename T, typename T2>
void test_F_sending_actives(NodeInfo<T, T2>& node)
{
	node.block_->get_F_actives_cpu();
	node.block_->update_F_sending_actives_cpu();
	{
		thrust::device_vector<unsigned int> d_f_sending_actives_rowptrs_cpu(node.block_->f_sending_rowptrs_->count());
		HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_f_sending_actives_rowptrs_cpu.data()), node.block_->f_sending_active_rowptrs_->cpu_data(), d_f_sending_actives_rowptrs_cpu.size() * sizeof(unsigned int), hipMemcpyHostToDevice));
		assert(thrust::equal(d_f_sending_actives_rowptrs_cpu.begin(), d_f_sending_actives_rowptrs_cpu.end(), node.block_->f_sending_active_rowptrs_->gpu_data()));
	}

	for(unsigned int j = 0; j < node.block_->f_sending_rowptrs_->count() - 1; j++)
	{
		unsigned int count = node.block_->f_sending_active_rowptrs_->cpu_data()[j + 1] - node.block_->f_sending_active_rowptrs_->cpu_data()[j];
		if(count > 0)
		{
			unsigned int* begin = node.block_->f_sending_active_colinds_->mutable_gpu_data() + node.block_->f_sending_active_rowptrs_->cpu_data()[j];
			unsigned int* end = node.block_->f_sending_active_colinds_->mutable_gpu_data() + node.block_->f_sending_active_rowptrs_->cpu_data()[j + 1];
			unsigned int* new_end = thrust::unique(begin, end); 
			assert(new_end == end);
		}
	}

	unsigned int count = node.block_->f_sending_active_rowptrs_->cpu_data()[node.block_->f_sending_rowptrs_->count() - 1];
	thrust::device_vector<unsigned int> d_f_sending_actives_colinds_cpu(count);
	HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_f_sending_actives_colinds_cpu.data()), node.block_->f_sending_active_colinds_->cpu_data(), count * sizeof(unsigned int), hipMemcpyHostToDevice));
	assert(thrust::equal(d_f_sending_actives_colinds_cpu.begin(), d_f_sending_actives_colinds_cpu.end(), node.block_->f_sending_active_colinds_->gpu_data()));
	
	LOG_INFO << "check update_F_sending_actives_gpu done...." << std::endl;
}

template<typename T, typename T2>
void test_F_recving_actives(NodeInfo<T, T2>& node,
							const unsigned int recv_count,
							const unordered_map<int, tuple<int, vector<unsigned char>>>& records,
							...
							Notification& recving_notification)
{
	...
	if(!node.trans_table_->recv_requests_.empty())
	{
		...
		for(unsigned int j = 0; j < recv_count; j++)
		{
			auto tag = node.recving_queue_.pop();

			auto record_it = records.find(tag);
			assert(record_it != records.end());
			auto rank = std::get<0>(record_it->second);
			const auto& f_actives = std::get<1>(record_it->second);
			
			auto recv_it = node.trans_table_->recv_table_.find(tag);
			if(recv_it != node.trans_table_->recv_table_.end())
			{
				bool found = false;
				for(unsigned int k = 0; k < node.block_->f_receiving_bids_.size(); k++)
				{
					if(tag == ID2RANK(node.block_->f_receiving_bids_[k]))
					{
						vector<unsigned int> f_active_colinds;	
						for(unsigned int m = node.block_->f_receiving_rowptrs_->cpu_data()[k]; 
								m < node.block_->f_receiving_rowptrs_->cpu_data()[k + 1]; m++)
						{
							unsigned int fidx = node.block_->f_receiving_colinds_->cpu_data()[m];
							if(f_actives[fidx])
							{
								f_active_colinds.push_back(m);
							}
						}

						
						unsigned int active_num = node.block_->f_receiving_active_rowptrs_->cpu_data()[k + 1] - node.block_->f_receiving_active_rowptrs_->cpu_data()[k];
						assert(active_num == f_active_colinds.size());
						if(active_num > 0)
						{
							unsigned int* d_colinds = node.block_->f_receiving_active_colinds_->mutable_gpu_data() + node.block_->f_receiving_active_rowptrs_->cpu_data()[k];
							unsigned int* end = node.block_->f_receiving_active_colinds_->mutable_gpu_data() + node.block_->f_receiving_active_rowptrs_->cpu_data()[k + 1];
							unsigned int* new_end = thrust::unique(d_colinds, end); 
							assert(new_end == end);
							
							thrust::device_vector<unsigned int> d_active_colinds = f_active_colinds;
							assert(thrust::equal(d_active_colinds.begin(),  d_active_colinds.end(), d_colinds));
						}
				...
				if(TransTable::MemoryType::MEMORY_CPU == recv_it->second.type_)
				{
					continue;
				}
			}

			auto route_it = node.trans_table_->route_table_.find(tag);
			assert(route_it != node.trans_table_->route_table_.end());
			assert(rank == route_it->second.sender_rank_);

			unsigned int count = route_it->second.sender_active_colinds_->count();
			vector<unsigned int> f_active_colinds;
					
			for(unsigned int k = 0; k < count; k++)
			{
				unsigned int fidx = route_it->second.sender_active_colinds_->cpu_data()[k];
				if(f_actives[fidx])
				{
					f_active_colinds.push_back(k);
				}
			}

			assert(f_active_colinds.size() == route_it->second.sender_count_);
			if(route_it->second.sender_count_ > 0)
			{
				unsigned int* d_colinds = route_it->second.sender_active_colinds_->mutable_gpu_data();
	...
	LOG_INFO << "check test_F_recving_actives done...." << std::endl;
}

template<typename T, typename T2>
void test_F_routing_actives(NodeInfo<T, T2>& node,
							TransTable::RouteRecord& record)
{
	thrust::host_vector<unsigned int> h_sender_active_colinds_gpu(record.sender_count_);
	HIP_CHECK(hipMemcpy(h_sender_active_colinds_gpu.data(),
						record.sender_active_colinds_->gpu_data(),
						record.sender_count_ * sizeof(unsigned int),
						hipMemcpyDeviceToHost));

	node.block_->update_F_routing_spikes_cpu(h_sender_active_colinds_gpu.data(),
											record.sender_count_,
											record.f_actives_->count(),
											record.f_actives_->mutable_cpu_data());
	
	node.block_->update_F_routing_actives_cpu(record.f_actives_->cpu_data(),
											record.receiver_rowptrs_->cpu_data(),
											record.receiver_colinds_->cpu_data(),
											record.receiver_rowptrs_->count() - 1,
											record.receiver_active_rowptrs_->mutable_cpu_data(),
											record.receiver_active_colinds_->mutable_cpu_data());
	
	{
		thrust::device_vector<unsigned char> d_f_actvies_cpu(record.f_actives_->count());

		HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_f_actvies_cpu.data()), 
							record.f_actives_->cpu_data(),
							record.f_actives_->size(),
							hipMemcpyHostToDevice));
		
		assert(thrust::equal(d_f_actvies_cpu.begin(), d_f_actvies_cpu.end(), record.f_actives_->gpu_data()));
	}

	{
		thrust::device_vector<unsigned int> d_receiver_active_rowptrs_cpu(record.receiver_rowptrs_->count());

		HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_receiver_active_rowptrs_cpu.data()),
						record.receiver_active_rowptrs_->cpu_data(),
						record.receiver_rowptrs_->size(),
						hipMemcpyHostToDevice));
		
		assert(thrust::equal(d_receiver_active_rowptrs_cpu.begin(), d_receiver_active_rowptrs_cpu.end(), record.receiver_active_rowptrs_->gpu_data()));
	}

	for(unsigned int j = 0; j < record.receiver_rowptrs_->count() - 1; j++)
	{
		unsigned int count = record.receiver_active_rowptrs_->cpu_data()[j + 1] - record.receiver_active_rowptrs_->cpu_data()[j];
		if(count > 0)
		{
			unsigned int* begin = record.receiver_active_colinds_->mutable_gpu_data() + record.receiver_active_rowptrs_->cpu_data()[j];
			unsigned int* end = record.receiver_active_colinds_->mutable_gpu_data() + record.receiver_active_rowptrs_->cpu_data()[j + 1];
			unsigned int* new_end = thrust::unique(begin, end); 
			assert(new_end == end);
		}
	}

	unsigned int count = record.receiver_active_rowptrs_->cpu_data()[record.receiver_rowptrs_->count() - 1];
	thrust::device_vector<unsigned int> d_receiver_active_colinds_cpu(count);
	HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_receiver_active_colinds_cpu.data()), 
							record.receiver_active_colinds_->cpu_data(),
							count * sizeof(unsigned int),
							hipMemcpyHostToDevice));

	assert(thrust::equal(d_receiver_active_colinds_cpu.begin(), d_receiver_active_colinds_cpu.end(), record.receiver_active_colinds_->gpu_data()));
	LOG_INFO << "check update_F_routing_actives_gpu done...." << std::endl;
}

template<typename T, typename T2>
void test_J_presynaptics(NodeInfo<T, T2>& node, const int iter)
{
	if(node.block_->block_type_ == BrainBlock<T, T2>::BLOCK_TYPE::BLOCK_TYPE_NORMAL)
	{
		vector<T2> j_presynaptics_gpu(node.block_->get_total_neurons());
		
		node.block_->update_J_presynaptic_inner_cpu();
		node.block_->update_J_presynaptic_outer_cpu();
		node.block_->update_J_presynaptic_cpu();
		
		HIP_CHECK(hipMemcpy(j_presynaptics_gpu.data(), node.block_->get_J_ex_presynaptics_gpu(), j_presynaptics_gpu.size() * sizeof(T2), hipMemcpyDeviceToHost));
		float max_err = FLT_MIN;
		const T2* j_presynaptics_cpu = node.block_->get_J_ex_presynaptics_cpu(false);
		for(unsigned int j = 0; j < j_presynaptics_gpu.size(); j++)
		{
			float diff = fabsf(j_presynaptics_cpu[j].x - j_presynaptics_gpu[j].x) / fabsf(j_presynaptics_cpu[j].x + FLT_MIN);
			if(diff > max_err)
			{
				max_err = diff;
			}
			
			diff = fabsf(j_presynaptics_cpu[j].y - j_presynaptics_gpu[j].y) / fabsf(j_presynaptics_cpu[j].y + FLT_MIN);
			if(diff > max_err)
			{
				max_err = diff;
			}
		...
		if(max_err > FLT_MIN)
		{
			LOG_INFO << "the max relative error of the " << iter << "th excit J presynaptic: " << max_err << endl;
		}
		else
		{
			LOG_INFO << "the max relative error of the " << iter << "th excit J presynaptic: 0" << endl;
		}

		max_err = FLT_MIN;
		j_presynaptics_cpu = node.block_->get_J_in_presynaptics_cpu(false);
		HIP_CHECK(hipMemcpy(j_presynaptics_gpu.data(), node.block_->get_J_in_presynaptics_gpu(), j_presynaptics_gpu.size() * sizeof(T2), hipMemcpyDeviceToHost));
		for(unsigned int j = 0; j < j_presynaptics_gpu.size(); j++)
		{
			float diff = fabsf(j_presynaptics_cpu[j].x - j_presynaptics_gpu[j].x) / fabsf(j_presynaptics_cpu[j].x + FLT_MIN);
			if(diff > max_err)
		...
		if(max_err > FLT_MIN)
		{
			LOG_INFO << "the max relative error of the " << iter << "th inhbit J presynaptic: " << max_err << endl;
		}
		else
		{
			LOG_INFO << "the max relative error of the " << iter << "th inhbit J presynaptic: 0" << endl;
		}
	}
}

template<typename T, typename T2>
void test_I_synaptics(NodeInfo<T, T2>& node, const int iter)
{
	if(BrainBlock<T, T2>::BLOCK_TYPE::BLOCK_TYPE_NORMAL == node.block_->block_type_)
	{
		vector<T> i_synaptics_gpu(node.block_->get_total_neurons());
		node.block_->update_I_synaptic_cpu();
		HIP_CHECK(hipMemcpy(i_synaptics_gpu.data(), node.block_->get_I_synaptics_gpu(), i_synaptics_gpu.size() * sizeof(T), hipMemcpyDeviceToHost));
		float max_err = FLT_MIN;
		unsigned int max_idx;
		const T* i_synaptics_cpu = node.block_->get_I_synaptics_cpu(false);
		
		for(unsigned int j = 0; j < i_synaptics_gpu.size(); j++)
		{
			float diff = fabsf(i_synaptics_cpu[j] - i_synaptics_gpu[j]) / fabsf(i_synaptics_cpu[j] + FLT_MIN);
			if(diff > max_err)
			{
				max_err = diff;
				max_idx = j;
		...
		{
			std::stringstream ss;
			ss << "the max relative error of the " << iter << "th I synaptic: " << max_err << endl;
			ss << "cpu result: " << i_synaptics_cpu[max_idx] << " vs. gpu result: " << i_synaptics_gpu[max_idx] << endl;
			LOG_INFO << ss.str();
		}
		else
		{
			LOG_INFO << "the max relative error of the " << iter << "th I synaptic: 0" << endl;
		}
	}
}

void test_union_route(const thrust::device_vector<unsigned int>& d_unions,
						const unsigned int union_size,
						const TransTable::Config::RouteInfo& route)
{
	
	set<unsigned int> unions;
	vector<unsigned int> h_unions(union_size);
	HIP_CHECK(hipMemcpy(h_unions.data(), thrust::raw_pointer_cast(d_unions.data()), union_size * sizeof(unsigned int), hipMemcpyDeviceToHost));

	//thrust::device_vector<unsigned int>::iterator iter;
	unsigned int* new_end = thrust::unique(h_unions.data(), h_unions.data() + union_size);
	assert((new_end - h_unions.data()) == union_size);
	
	for(auto it = route.begin(); it != route.end(); ++it)
	{
		for(int idx = 0; idx < std::get<1>(it->second); idx++)
		{
			unions.insert(std::get<0>(it->second)[idx]);
		}
	}
	
	assert(unions.size() == union_size);

	unsigned int idx = 0;
	for(auto it = unions.begin(); it != unions.end(); it++, idx++)
	{
		assert(*it == h_unions[idx]);
	}
}

template<typename T, typename T2>
void test_F_routing_offsets(const NodeInfo<T, T2>& node,
							const TransTable::RouteRecord& record)
{
	node.block_->update_F_routing_offsets_cpu(record.sender_active_colinds_->cpu_data(),
										record.sender_active_colinds_->count(),
										record.receiver_ranks_.size(),
										record.receiver_rowptrs_->cpu_data(),
										record.receiver_colinds_->mutable_cpu_data());

	thrust::device_vector<unsigned int> d_receiver_colinds(record.receiver_colinds_->count());
	HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_receiver_colinds.data()), 
						record.receiver_colinds_->cpu_data(), 
						record.receiver_colinds_->size(),
						hipMemcpyHostToDevice));
	assert(thrust::equal(d_receiver_colinds.begin(), d_receiver_colinds.end(), record.receiver_colinds_->gpu_data()));

	LOG_INFO << "check compute_F_routing_offsets_gpu done...." << std::endl;
}

template<typename T, typename T2>
void test_stat_results(const NodeInfo<T, T2>& node, const int iter)
{
	if(node.has_freq_)
	{
		node.block_->stat_Freqs_cpu();
		thrust::device_vector<unsigned int> d_freqs(node.block_->get_total_subblocks());
		HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_freqs.data()), 
						node.block_->get_freqs_cpu(false), 
						d_freqs.size() * sizeof(unsigned int),
						hipMemcpyHostToDevice));
		assert(thrust::equal(d_freqs.begin(), d_freqs.end(), node.block_->get_freqs_gpu()));
	}

	if(node.has_vmean_ || node.has_imean_)
	{
		node.block_->stat_Vmeans_and_Imeans_cpu(node.has_vmean_, node.has_imean_);
	}
	
	if(node.has_vmean_)
	{
		thrust::host_vector<T> h_vmeans_gpu(node.block_->get_total_subblocks());
		HIP_CHECK(hipMemcpy(h_vmeans_gpu.data(), 
						node.block_->get_vmeans_gpu(), 
						h_vmeans_gpu.size() * sizeof(T),
						hipMemcpyDeviceToHost));
		
		float max_err = FLT_MIN;
		unsigned int max_idx;
		const T* vmeans_cpu = node.block_->get_vmeans_cpu(false);
		
		for(unsigned int j = 0; j < h_vmeans_gpu.size(); j++)
		{
			float diff = fabsf(vmeans_cpu[j] - h_vmeans_gpu[j]) / fabsf(vmeans_cpu[j] + FLT_MIN);
			if(diff > max_err)
			{
				max_err = diff;
				max_idx = j;
		...
		if(max_err > FLT_MIN)
		{
			LOG_INFO << "the max relative error of the " << iter << "th vmeans: " << max_err << endl;
			LOG_INFO << "cpu result: " << vmeans_cpu[max_idx] << " vs. gpu result: " << h_vmeans_gpu[max_idx] << endl;
		}
		else
		{
			LOG_INFO << "the max relative error of the " << iter << "th vmeans: 0" << endl;
		}
	}
	
	if(node.has_imean_)
	{
		thrust::host_vector<T> h_imeans_gpu(node.block_->get_total_subblocks());
		HIP_CHECK(hipMemcpy(h_imeans_gpu.data(), 
						node.block_->get_imeans_gpu(), 
						h_imeans_gpu.size() * sizeof(unsigned int),
						hipMemcpyDeviceToHost));
		float max_err = FLT_MIN;
		unsigned int max_idx;
		const T* imeans_cpu = node.block_->get_imeans_cpu(false);
		
		for(unsigned int j = 0; j < h_imeans_gpu.size(); j++)
		{
			float diff = fabsf(imeans_cpu[j] - h_imeans_gpu[j]) / fabsf(imeans_cpu[j] + FLT_MIN);
			if(diff > max_err)
			{
				max_err = diff;
				max_idx = j;
		...
		{
			std::stringstream ss;
			ss << "the max relative error of the " << iter << "th imeans: " << max_err << endl;
			ss << "cpu result: " << imeans_cpu[max_idx] << " vs. gpu result: " << h_imeans_gpu[max_idx] << endl;
			LOG_INFO << ss.str();
		}
		else
		{
			LOG_INFO << "the max relative error of the " << iter << "th imeans: 0" << endl;
		}
	}
	
	LOG_INFO << "check statistic results done...." << std::endl;
}

template<typename T, typename T2>
void test_send(NodeInfo<T, T2>& node, std::vector<int>& send_indice, const int iter)
{
	int err;
	vector<unsigned char> f_actives(node.block_->get_total_neurons());
	memcpy(f_actives.data(), node.block_->get_F_actives_cpu(false), f_actives.size());
	for(unsigned int j = 0; j < send_indice.size(); j++)
	{
		int index = send_indice[j];
		const auto& meta = node.trans_table_->send_buffs_[index];
		
		err = MPI_Isend(f_actives.data(),
						f_actives.size(),
						MPI_CHAR,
						meta.receiver_rank_,
						node.info_->rank_,
	...
	err = MPI_Waitall(node.trans_table_->send_requests_.size(),
	...
	{
		const auto& meta = node.trans_table_->send_buffs_[j];
		LOG_VERBOSE(1) << "th" << iter << " send active flags to " << meta.receiver_rank_ << ": " << f_actives.size() << std::endl;
		assert(node.trans_table_->send_requests_[j] == MPI_REQUEST_NULL);
	}
}

template<typename T, typename T2>
void test_recv(NodeInfo<T, T2>& node,
				const int iter,
				const int recv_count, 
				unordered_map<int, tuple<int, vector<unsigned char>>>& recv_records)
{
	
	
	MPI_Status status;
	int err, rank, tag, elems;
	assert(node.recving_queue_.empty());
	assert(recv_count > 0);
	...
	for(int j = 0; j < recv_count; j++)
	{
		...
		LOG_VERBOSE(1) << "th" << iter << " recv active flags from " << rank << " [" << tag << "]: "  << elems << std::endl;
		auto recv_it = recv_records.find(tag);
		assert(recv_it != recv_records.end());
		
		if(0 == std::get<1>(recv_it->second).size())
		{
			std::get<0>(recv_it->second) = rank;
			std::get<1>(recv_it->second).resize(elems);
		}
		else
		{
			assert(std::get<0>(recv_it->second) == rank);
			assert((int)std::get<1>(recv_it->second).size() == elems);
		}
		
		err = MPI_Recv(std::get<1>(recv_it->second).data(), elems, MPI_CHAR, rank, tag, node.info_->comm_, MPI_STATUSES_IGNORE);
		assert(err == MPI_SUCCESS);
		
		node.recving_queue_.push(tag);
	}
}

template<typename T, typename T2>
void test_route(NodeInfo<T, T2>& node,
				const int iter,
				...
				unordered_map<int, tuple<int, vector<unsigned char>>>& recv_records)
{
	int check_count = 0;
	int err;
	assert(0 < route_count);
	vector<MPI_Request> requests(route_count);
	
	for(int j = 0; j < route_count;)
	{
		auto tag = node.routing_queue_.pop();

		auto route_it = node.trans_table_->route_table_.find(tag);
		assert(route_it != node.trans_table_->route_table_.end());
		auto record_it = recv_records.find(tag);
		assert(record_it != recv_records.end());

		for(auto receiver_rank : route_it->second.receiver_ranks_)
		{
			if(node.info_->rank_ != receiver_rank)
			{	
				err = MPI_Isend(std::get<1>(record_it->second).data(),
								std::get<1>(record_it->second).size(),
								MPI_CHAR,
								receiver_rank,
								tag,
								node.info_->comm_,
				...
				LOG_VERBOSE(1) << "th" << iter << " route active flags to " << receiver_rank << " [" << tag << "]: " << std::get<1>(record_it->second).size() << std::endl;
			}
		}
	}

	assert(check_count == route_count);
							...
							vector<int>& subcounts)
{
	...
	err = snn_gather(info, &neurons, 1, MPI_INT, neurons_per_block, 1, MPI_INT);
	assert(err == MPI_SUCCESS);
	
	err = snn_gather(info, &subblocks, 1, MPI_INT, recvcounts, 1, MPI_INT);
	assert(err == MPI_SUCCESS);
	...
	err = snn_gatherv(info, NULL, subblocks, MPI_INT, subids.data(), recvcounts, displs, MPI_INT);
	assert(err == MPI_SUCCESS);

	err = snn_gatherv(info, NULL, subblocks, MPI_INT, subcounts.data(), recvcounts, displs, MPI_INT);
	assert(err == MPI_SUCCESS);
...

{
	...
	node.block_->init_all_params_and_stages_cpu();
	test_I_synaptics(node, -1);
	node.block_->reset_V_membrane_cpu();
	#endif
...
{
	const unsigned short* receiver_bids = nullptr;
	const unsigned int* receiver_rowptrs = nullptr;
	const unsigned int* receiver_colinds = nullptr;
	...
	for (auto recv_it = conf.recving_ranks_.begin(); recv_it != conf.recving_ranks_.end();)
	{
		...
		if(conf.routing_ranks_.empty() || route_it == conf.routing_ranks_.end())
		{
			...
			#if DEBUG
			LOG_INFO << "[send_init]: sending to " << recv_it->second << " [" << recv_it->first << "] " << std::get<1>(sending_buffs.back()) << std::endl;
			#endif
			MPI_Request request;
		...
		else
		{
			...
			{
				#if DEBUG
				LOG_INFO << "[send_init]: record route info [" << recv_it->first << ":" << node.info_->rank_ << "], count: " << count << std::endl; 
				#endif
						...
						TransTable::RouteRecord& record)
{
	...
	#if DEBUG
	test_union_route(d_unions, union_size, route_info);
	#endif
	...
	#if DEBUG
	HIP_CHECK(hipMemcpy(record.receiver_colinds_->mutable_cpu_data(), record.receiver_colinds_->gpu_data(), record.receiver_colinds_->size(), hipMemcpyDeviceToHost));
	#endif
					...
					map<int, set<unsigned int>>& route_indice)
{
	...
	if(0 == recving_buffs[buff_index].size())
	{
			...
			std::get<2>(route_it->second)))
		{
			...
			#if DEBUG
				LOG_INFO << "[route_init]: sending to " << receiver_rank << " [" << tag << "] " << std::get<1>(info) << std::endl;
			#endif
	...
	if(info_it->second.size() == std::get<1>(route_it->second).size())
	{
		...
		if(1 < info_it->second.size())
		{
			...
			#if DEBUG
			test_F_routing_offsets<T, T2>(node, tab_it->second);
			#endif
		...
		else
		{
			...
			#if DEBUG
			tab_it->second.sender_active_colinds_ = make_unique<DataAllocator<unsigned int>>(node.info_->rank_, sizeof(unsigned int) * std::get<1>(info_it->second.begin()->second));
			memcpy(tab_it->second.sender_active_colinds_->mutable_cpu_data(), std::get<0>(info_it->second.begin()->second), tab_it->second.sender_active_colinds_->size());
			#endif
		...
		#if DEBUG
		LOG_INFO << "[route_init]: routing to " << recv_it->second << " [" << tag << "] " << std::get<1>(sending_buffs.back()) << std::endl;
		#endif
					...
					vector<TransTable::BufferRecord>& sending_buffs)
{
	...
	{	
		...
		#if DEBUG
		LOG_INFO << "[recv_init]: recving from " << rank << "[" << tag << "] " << elems << std::endl;
		#endif
							...
							TransTable::RecvRecord::MergeRecord* merge_record = nullptr)
{
			...
			rank_it++, idx++)
	{
		...
		if(*rank_it == node.info_->rank_)
		{
			...
			#if DEBUG
			{
				thrust::host_vector<unsigned int> h_sender_active_colinds(route_it->second.sender_active_colinds_->count());
				HIP_CHECK(hipMemcpy(h_sender_active_colinds.data(),
									route_it->second.sender_active_colinds_->gpu_data(),
									route_it->second.sender_active_colinds_->size(),
									hipMemcpyDeviceToHost));
				HIP_CHECK(hipMemcpy(h_receiver_colinds.data(),
									route_it->second.receiver_colinds_->gpu_data() + route_it->second.receiver_rowptrs_->cpu_data()[idx],
									h_receiver_colinds.size() * sizeof(unsigned int),
									hipMemcpyDeviceToHost));
				node.block_->update_F_routing_neuron_ids_cpu(h_sender_active_colinds.data(),
															h_receiver_colinds.size(),
															h_receiver_colinds.data());
				thrust::device_vector<unsigned int> d_data(h_receiver_colinds.size());
				HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_data.data()),
						h_receiver_colinds.data(),
						d_data.size() * sizeof(unsigned int),
						hipMemcpyHostToDevice));
				assert(thrust::unique(d_receiver_colinds.begin(), d_receiver_colinds.end()) == d_receiver_colinds.end());
				assert(thrust::unique(d_data.begin(), d_data.end()) == d_data.end());
				assert(thrust::equal(d_data.begin(), d_data.end(), d_receiver_colinds.begin()));
			}
			#endif
		...
		{	
			...
			if(rank != route_meta.receiver_tag_)
			{
				...
				#if DEBUG
				LOG_INFO << "[validate_route]: send to " << *rank_it << "[" << rank << "]" << " null package." << std::endl;
				#endif
				MPI_Request request;
			...
			{
				#if DEBUG
				LOG_INFO << "[validate_route]: merge all packet done." << std::endl;
				#endif
				...
				#if DEBUG
				LOG_INFO << "[validate_route]: routing to " << route_meta.receiver_rank_ << " [" << route_meta.receiver_tag_ << "] " << std::get<1>(route_buff) << std::endl;
				#endif
		...
		else
		{
			...
			{
				#if DEBUG
				LOG_INFO << "[validate_route]: there is route union." << std::endl;
				#endif
			...
			{
				#if DEBUG
				LOG_INFO << "[validate_route]: only route forward." << std::endl;
				#endif
...
static void validate_init(NodeInfo<T, T2>& node)
{
	...
	if(!node.trans_table_->send_requests_.empty())
	{
		...
		{	
			...
			#if DEBUG
			LOG_INFO << "[validate_init]: sending to " << meta.receiver_rank_ << " [" << node.info_->rank_ << "] " << std::get<1>(send_buff) << std::endl;
			#endif
	...
	if(!node.trans_table_->recv_requests_.empty())
	{
		...
		#if DEBUG
		LOG_INFO << "[validate init] recv request size: " << node.trans_table_->recv_requests_.size() << std::endl;
		#endif
		...
		{	
			...
			#if DEBUG
			LOG_INFO << "[validate_init]: recving from " << rank << "[" << tag << "] " << elems << std::endl;
			#endif
			...
			switch(node.trans_table_->mode_)
			{
				case TransTable::Mode::MODE_ROUTE_WITH_MERGE:
					...
					{
						#if DEBUG
						LOG_INFO << "[validate_init]: recv merging from " << rank << " [" << tag << "] " << elems << std::endl;
						#endif
...
static void snn_init(NodeInfo<T, T2>& node)
{
	...
	#if !defined(DEBUG)
	if(node.block_->f_receiving_colinds_ != nullptr)
						...
						const TransTable::BufferRecord& buff)
{
	...
	if(1 < route_it->second.receiver_ranks_.size())
	{
		...
		if(route_it->second.sender_count_ > 0)
		{
			HIP_CHECK(hipMemcpyAsync(route_it->second.sender_active_colinds_->mutable_gpu_data(), 
									std::get<0>(buff),
			...
			#if DEBUG
			test_F_routing_actives(node, route_it->second);
			#else
			HIP_CHECK(hipMemcpyAsync(route_it->second.receiver_active_rowptrs_->mutable_cpu_data(), 
	...
	{
		//LOG_INFO << "[route_routine]: has pure route." << std::endl;
		#if DEBUG
		route_it->second.sender_count_ = static_cast<unsigned int>(std::get<1>(buff));
		assert(route_it->second.sender_count_ <= route_it->second.sender_active_colinds_->count());
		if(route_it->second.sender_count_ > 0)
						...
						for(auto& buff_record : route.buff_records_)
						{
							...
							switch(std::get<3>(buff_record))
							{
								case TransTable::MemoryType::MEMORY_GPU:
									HIP_CHECK(hipMemcpyAsync(std::get<0>(route.buffer_)->mutable_cpu_data() + offset,
				...
				if(has_union)
				{
					...
					if(0 < std::get<1>(buff))
					{
						...
						if(0 < count)
						{
							...
							HIP_CHECK(hipMemcpyAsync(std::get<0>(route.buffer_)->mutable_cpu_data(),
						...
						const std::chrono::milliseconds& timeout)
{
	...
	if(timeout == kNoTimeout)
	{
        ...
        for(unsigned int j = 0; j < node.trans_table_->send_requests_.size(); j++)
        {
	        assert(node.trans_table_->send_requests_[j] == MPI_REQUEST_NULL);
			const auto& meta = node.trans_table_->send_buffs_[j];
			LOG_VERBOSE(1) << " send to " << meta.receiver_rank_ << ": " << std::get<1>(meta.buffer_) << std::endl;
        }
		
		return;
	}

	std::unordered_set<unsigned int> cancelled_requests;
	MPI_Status status;
	...
	while(true)
	{
		...
		const auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(diff);
		
		for(unsigned int j = 0; j < node.trans_table_->send_requests_.size(); j++)
		{
			if (node.trans_table_->send_requests_[j] != MPI_REQUEST_NULL)
			{
				int flag = 0;
				auto it = cancelled_requests.find(j);
				const auto& meta = node.trans_table_->send_buffs_[j];
				assert(MPI_SUCCESS == MPI_Test(&node.trans_table_->send_requests_[j], &flag, &status));

				if(flag)
				{
					if(it == cancelled_requests.end())
					{
						...
						else
						{
							...
							int len = sizeof(buf);
							assert(MPI_SUCCESS == MPI_Error_string(status.MPI_ERROR, buf, &len));
								
							LOG_ERROR << "send error: [" << meta.receiver_rank_ << "] " << std::get<1>(meta.buffer_) << " (" << std::string(buf, len) << ")" << std::endl;
						}
					}
					else
					{
						LOG_ERROR << "send timeout: [" << meta.receiver_rank_ << "] " << std::get<1>(meta.buffer_) << std::endl;

					}

					time_start = steady_clock::now();
				}
				else if(it == cancelled_requests.end())
				{
					assert(MPI_SUCCESS == MPI_Test_cancelled(&status, &flag));
					if(flag || elapsed > timeout)
					{
						assert(MPI_SUCCESS == MPI_Cancel(&node.trans_table_->send_requests_[j]));
						assert(cancelled_requests.insert(j).second);
					}
				}
			}
			else
		...
		if(completed_requests == node.trans_table_->send_requests_.size())
		{
			break;
		}
						...
						const std::chrono::milliseconds& timeout)
{
	...
	for(unsigned int j = 0; j < node.trans_table_->recv_requests_.size(); j++)
	{
		...
		int err = MPI_Irecv(std::get<0>(meta.buffer_)->mutable_cpu_data(),
	...
	if(timeout == kNoTimeout)
	{
		...
		for(unsigned int j = 0; j < node.trans_table_->recv_requests_.size(); j++)
	    {
            err = MPI_Waitany(node.trans_table_->recv_requests_.size(),
            ...
            assert(meta.sender_rank_ == status.MPI_SOURCE);
            assert(meta.sender_tag_ == status.MPI_TAG);

            err = MPI_Get_count(&status, MPI_UNSIGNED, &elems);
            assert(err == MPI_SUCCESS && elems <= std::get<0>(meta.buffer_)->count());
            std::get<1>(meta.buffer_) = elems;
            
            LOG_VERBOSE(1) << "recv from " << meta.sender_rank_ << " [" << meta.sender_tag_ << "]: "  << std::get<1>(meta.buffer_) << std::endl;
		...
		for(unsigned int j = 0; j < node.trans_table_->recv_requests_.size(); j++)
	    {
            assert(node.trans_table_->recv_requests_[j] == MPI_REQUEST_NULL);
	    }
	...
	while(true)
	{
		...
		for(unsigned int j = 0; j < node.trans_table_->recv_requests_.size(); j++)
		{
			if (node.trans_table_->recv_requests_[j] != MPI_REQUEST_NULL)
			{
				int flag = 0;
				auto it = cancelled_requests.find(j);
				auto& meta = node.trans_table_->recv_buffs_[j];
				assert(MPI_SUCCESS == MPI_Test(&node.trans_table_->recv_requests_[j], &flag, &status));
				
				if (flag)
				{
					if(it == cancelled_requests.end())
					{
						assert(MPI_REQUEST_NULL == node.trans_table_->recv_requests_[j]);
						...
						else
						{
							...
							LOG_ERROR << "recv error: [" << meta.sender_rank_ << "] " << std::string(buf, len) << std::endl;
							std::get<1>(meta.buffer_) = 0;
						}
					}
					else
					{
						LOG_ERROR << "recv timeout: [" << meta.sender_rank_ << "]" << std::endl;
						std::get<1>(meta.buffer_) = 0;
					}
					
					node.recving_queue_.push(static_cast<int>(j));
					time_start = steady_clock::now();
				}
				else if(it == cancelled_requests.end() && elapsed > timeout)
				{
					assert(MPI_SUCCESS == MPI_Cancel(&node.trans_table_->recv_requests_[j]));
					assert(cancelled_requests.insert(j).second);
				}
			}
			else
		...
		if(completed_requests == node.trans_table_->recv_requests_.size())
		{
			break;
		}
						...
						const std::chrono::milliseconds& timeout)
{
	...
	if(timeout == kNoTimeout)
	{
		err = MPI_Waitall(node.trans_table_->route_requests_.size(),
                        node.trans_table_->route_requests_.data(),
                        MPI_STATUSES_IGNORE);
        assert(err == MPI_SUCCESS);

        for(unsigned int j = 0; j < node.trans_table_->route_requests_.size(); j++)
        {
            assert(node.trans_table_->route_requests_[j] == MPI_REQUEST_NULL);
			auto& meta = node.trans_table_->route_buffs_[j];
			LOG_VERBOSE(1) << "route to " << meta.receiver_rank_ << " [" << meta.receiver_tag_ << "]: "  << std::get<1>(meta.buffer_) << std::endl;
        }
	...
	while(true)
	{
		...
		for(unsigned int j = 0; j < node.trans_table_->route_requests_.size(); j++)
		{
			if (node.trans_table_->route_requests_[j] != MPI_REQUEST_NULL)
			{
				int flag = 0;
				auto it = cancelled_requests.find(j);
				const auto& meta = node.trans_table_->route_buffs_[j];
				assert(MPI_SUCCESS == MPI_Test(&node.trans_table_->route_requests_[j], &flag, &status));
				...
				if(flag)
				{
					if(it == cancelled_requests.end())
					{
						...
						else
						{
							...
							LOG_ERROR << "route error: [" << meta.receiver_rank_ << "] " << std::get<1>(meta.buffer_) << " (" << std::string(buf, len) << ")" << std::endl;
						}
					}
					else
					{
						LOG_ERROR << "route timeout: [" << meta.receiver_rank_ << "] " << std::get<1>(meta.buffer_) << std::endl;
				...
				else if(it == cancelled_requests.end())
				{
					...
					if(flag || elapsed > timeout)
					{
						assert(MPI_SUCCESS == MPI_Cancel(&node.trans_table_->route_requests_[j]));
						assert(cancelled_requests.insert(j).second);
					}
				}
			}
		...
		if(completed_requests == node.trans_table_->route_requests_.size())
		{
			break;
		}
...
static void snn_run(NodeInfo<T, T2>& node)
{
	...
	//Storing tag as key, and rank in first element, buffer in 2nd. 
	unordered_map<int, tuple<int, vector<unsigned char>>> recv_records;

	int recv_count = static_cast<int>(node.trans_table_->recv_table_.size());
	for(const auto& item : node.trans_table_->recv_table_)
	{
		auto record = recv_records.emplace(std::make_pair(item.first, tuple<int, vector<unsigned char>>()));
		assert(record.second);
	}
	int route_count = 0;
	for(const auto& pair : node.trans_table_->route_table_)
	{
		assert(pair.first != node.info_->rank_);
		const auto& rank_set = pair.second.receiver_ranks_;
		if(rank_set.find(node.info_->rank_) == rank_set.end())
		{
			route_count += static_cast<int>(rank_set.size());
			recv_count++;
			auto record = recv_records.emplace(std::make_pair(pair.first, tuple<int, vector<unsigned char>>()));
			assert(record.second);
		}
		else
		{
			route_count += (static_cast<int>(rank_set.size()) - 1);
		}
	}	
#endif
	{
		assert(thrust::count(node.block_->get_noise_rate_gpu(), node.block_->get_noise_rate_gpu() + node.block_->get_total_neurons(), 0.f) == 0);
		assert(thrust::count(node.block_->get_noise_rate_gpu(), node.block_->get_noise_rate_gpu() + node.block_->get_total_neurons(), 1.f) == 0);
	}
	...
	std::chrono::milliseconds timeout = std::chrono::seconds(node.timeout_);
	...
	{
		std::stringstream ss;
		ss << "send indice: " << std::endl;
		assert(send_indice.size() == node.trans_table_->send_requests_.size());
		for(unsigned int j = 0; j < send_indice.size(); j++)
		{
			int index = send_indice[j];
			const auto& meta = node.trans_table_->send_buffs_[index];
			ss << meta.receiver_rank_ << " ";
    ...
    if(!node.trans_table_->send_requests_.empty())
    {
        sending_thread = std::thread(
                [&node, &send_indice, &timeout, &sending_notification, &sending_done_notification]()
		{
			for(int i = 0; i < node.iter_; i++)
            {
               sending_notification.Wait();
			   snn_send(node, send_indice, i, timeout);
			   #if DEBUG
			   sending_notification.Wait();
			   test_send(node, send_indice, i);
			   #endif
    ...
    if(!node.trans_table_->recv_requests_.empty())
    {
        ...
        recving_thread = std::thread(
                [&node, recv_count, &recv_records, &timeout, &recving_notification]()
        #else
        recving_thread = std::thread(
                [&node, &timeout, &recving_notification]()
        #endif
        {
            for(int i = 0; i < node.iter_; i++)
            {
               recving_notification.Wait();
			   snn_recv(node, i, timeout);
			   #if DEBUG
			   recving_notification.Wait();
			   test_recv(node, i, recv_count, recv_records);
			   #endif
			}
        });
    }
    ...
    if(!node.trans_table_->route_requests_.empty())
    {
        ...
        routing_thread = std::thread(
                [&node, &route_count, &recv_records, &timeout, &routing_notification, &routing_done_notification]()
        #else
        routing_thread = std::thread([&node, &timeout, &routing_notification, &routing_done_notification]()
        #endif
        {
	        for(int i = 0; i < node.iter_; i++)
            {
               routing_notification.Wait();
			   snn_route(node, i, timeout);
			   #if DEBUG
			   routing_notification.Wait();
			   test_route(node, i, route_count, recv_records);
			   #endif
	...
	for(int i = 0; i < node.iter_; i++)
	{
		...
		#if DEBUG
		node.block_->get_F_actives_without_noise_cpu();
		test_F_actives(node);
		#else
		...
		#if DEBUG
		test_F_sending_actives<T, T2>(node);
		#endif
		...
		node.block_->update_J_presynaptic_inner_gpu(node.stream1_->get());

		if(!node.trans_table_->send_requests_.empty())
		{
			HIP_CHECK(hipMemcpyAsync(node.block_->f_sending_active_rowptrs_->mutable_cpu_data(), node.block_->f_sending_active_rowptrs_->gpu_data(), node.block_->f_sending_rowptrs_->size(), hipMemcpyDeviceToHost, node.stream2_->get()));
			HIP_CHECK(hipStreamSynchronize(node.stream2_->get()));
			unsigned int count = node.block_->f_sending_active_rowptrs_->cpu_data()[node.block_->f_sending_rowptrs_->count() - 1];
			if(count > 0)
			{
		...
		if(!node.trans_table_->recv_table_.empty())
		{
			...
			for(unsigned int j = 0; j < node.block_->f_receiving_bids_.size(); j++)
			{
				...
				assert(recv_it != node.trans_table_->recv_table_.end());
				int count = std::get<1>(recv_it->second.record_);
				assert(count >= 0);
				...
				if(count > 0)
				{
					...
					{
						case TransTable::MemoryType::MEMORY_CPU:
							memcpy(h_colinds, std::get<0>(recv_it->second.record_), static_cast<unsigned int>(count) * sizeof(unsigned int));
						break;
						case TransTable::MemoryType::MEMORY_GPU:
							HIP_CHECK(hipMemcpyAsync(h_colinds,
										std::get<0>(recv_it->second.record_),
										static_cast<unsigned int>(count) * sizeof(unsigned int),
										hipMemcpyDeviceToHost,
										node.stream2_->get()));
							HIP_CHECK(hipStreamSynchronize(node.stream2_->get()));
						break;
			...
			#if DEBUG
			assert(thrust::is_sorted(node.block_->f_receiving_active_rowptrs_->gpu_data(), node.block_->f_receiving_active_rowptrs_->gpu_data() + node.block_->f_receiving_active_rowptrs_->count()));	
			#endif
			...
			if(total > 0)
			{
				...
				HIP_CHECK(hipMemcpyAsync(node.block_->f_receiving_active_colinds_->mutable_gpu_data(),
				...
				#if DEBUG
				{
					node.block_->update_F_recving_actives_cpu();
					thrust::device_vector<unsigned int> d_active_colinds_cpu(total);
					HIP_CHECK(hipMemcpy(thrust::raw_pointer_cast(d_active_colinds_cpu.data()), node.block_->f_receiving_active_colinds_->cpu_data(), total * sizeof(unsigned int), hipMemcpyHostToDevice));
					assert(thrust::equal(d_active_colinds_cpu.begin(), d_active_colinds_cpu.end(), node.block_->f_receiving_active_colinds_->gpu_data()));
				}
				#endif
			}
		}

		HIP_CHECK(hipStreamSynchronize(node.stream1_->get()));

		#if DEBUG
		assert(snn_sync(info) == MPI_SUCCESS);
		test_F_recving_actives<T, T2>(node, recv_count, recv_records, sending_notification, routing_notification, recving_notification);
		#endif

		if(node.has_freq_)
		{
			HIP_CHECK(hipMemcpyAsync(report->freqs_->mutable_cpu_data(), node.block_->get_freqs_gpu(), report->freqs_->size(), hipMemcpyDeviceToHost, node.stream2_->get()));
		}
		...
		if(node.has_vmean_)
		{
			HIP_CHECK(hipMemcpyAsync(report->vmeans_->mutable_cpu_data(), node.block_->get_vmeans_gpu(), report->vmeans_->size(), hipMemcpyDeviceToHost, node.stream2_->get()));
		}
		...
		if(node.has_sample_ && nullptr != node.samples_)
		{
			HIP_CHECK(hipMemcpyAsync(report->spikes_->mutable_cpu_data(), node.spikes_->gpu_data(), report->spikes_->size(), hipMemcpyDeviceToHost, node.stream2_->get()));
			HIP_CHECK(hipMemcpyAsync(report->vmembs_->mutable_cpu_data(), node.vmembs_->gpu_data(), report->vmembs_->size(), hipMemcpyDeviceToHost, node.stream2_->get()));

		}
		
		node.block_->update_I_synaptic_gpu(node.stream1_->get());
		if(node.has_imean_)
		{
		...
		#if DEBUG
		test_J_presynaptics(node, i);
		test_I_synaptics(node, i);
		LOG_INFO << "================================================" << std::endl;
		#endif
		...
		#if DEBUG
		test_stat_results(node, node.iter_);
		#endif
...
static void snn_update_gamma(NodeInfo<T, T2>& node)
{
	...
	{
		std::stringstream ss;
		ss << "orginal brain id:" << std::endl;
		const unsigned int* subbids = node.block_->get_sub_bids_cpu();
		for(unsigned int i = 0; i < node.block_->get_sub_bids(); i++)
		{
			ss << subbids[i] << " ";
		}
		ss << std::endl;
	...
	if(elems > 0)
	{
		...
		{
			std::stringstream ss;
			ss << "brain id:" << std::endl;
			for(int i = 0; i < n; i++)
			{
				ss << node.brain_indice_[i] << " ";
			}
			ss << std::endl;
			LOG_INFO << ss.str();
		}
...
static void snn_update_hyperpara(NodeInfo<T, T2>& node)
{
	...
	if(elems > 0)
	{
		...
		const unsigned int* sbcounts = node.block_->get_sub_bcounts_cpu();
		const unsigned int* sbids = node.block_->get_sub_bids_cpu();
		for(int i = 0; i < elems; i++)
		{
			const unsigned int* iter = thrust::find(sbids, sbids + node.block_->get_total_subblocks(), brain_indice->cpu_data())[i]);
			unsigned int idx = iter - sbids;
			if(idx == node.block_->get_total_subblocks())
								...
								const int n)
{
	...
	if(n > 0)
	{
		vector<unsigned int> verified_vals(n);
		MPI_Status status;
		int elems;
		
		tag++;
		err = MPI_Recv(verified_vals.data(), n, MPI_UNSIGNED, rank, tag, info.comm_, &status);
		assert(err == MPI_SUCCESS);
		err = MPI_Get_count(&status, MPI_UNSIGNED, &elems);
		assert(err == MPI_SUCCESS && elems == n);

		for(int j = 0 ; j < n; j++)
			assert(verified_vals[j] == sample_indice[j]);
	}
	#endif
}
...
static void snn_update_sample(NodeInfo<T, T2>& node)
{
	...
	if(elems > 0)
	{
		...
		#if DEBUG
		tag++;
		err = MPI_Send(node.samples_->cpu_data(), elems, MPI_UNSIGNED, MPI_MASTER_RANK, tag, info.comm_);
		assert(err == MPI_SUCCESS);
		#endif
	}
	else
...
class SnnImpl final : public Snn::Service
{
 public:
	...
	Status Measure(ServerContext* context, const MetricRequest* request, ServerWriter<MetricResponse>* writer) override
	{
		...
		response.set_status(SnnStatus::SNN_UNSUPPORTED);
		return Status::OK;
	}
                     						...
                     						UpdateSampleResponse* response) override
    {
		...
		#ifdef DEBUG
		cout << "Number of samples to be set: " << total << endl;
		cout << "array of sending counts: "<< endl;
		for(int i = 0; i < info_->size_; i++)
		{
			cout << sample_recvcounts_[i] << " ";
		}
		cout << endl;

		cout << "array of displacements: "<< endl;
		for(int i = 0; i < info_->size_; i++)
		{
			cout << sample_displs_[i] << " ";
		}
		cout << endl;
		#endif
...
int main(int argc, char **argv)
{
	...
	{
		shared_ptr<NodeInfo<float, float2>> shrd_node = make_shared<NodeInfo<float, float2>>(rank, size, tag, comm, gpu_id, device, timeout);
