...
__global__ void transform_presynaptic_voltage_kernel(const T2* __restrict__ j_ex_presynaptics,
														const T2* __restrict__ j_in_presynaptics,
														const unsigned int n,
														T* __restrict__ j_presynaptics)
{
	...
	T2 ji_ex, ji_in;
  	...
  	{
  		ji_ex = j_ex_presynaptics[i];
		ji_in = j_in_presynaptics[i];
		
		j_presynaptics[i] = ji_ex.x;
		j_presynaptics[n + i] = ji_ex.y;
		j_presynaptics[2 * n + i] = ji_in.x;
		j_presynaptics[3 * n + i] = ji_in.y;
  	}
}
...
{
	hipLaunchKernelGGL(
					HIP_KERNEL_NAME(transform_presynaptic_voltage_kernel<T,T2>),
					...
					n,
					j_presynaptics);
	HIP_POST_KERNEL_CHECK("transform_presynaptic_voltage_kernel");
}
...
static __global__ void update_presynaptic_voltage_inner_kernel(const unsigned int* __restrict__ rowptrs,
																	...
																	const T2* __restrict__ w_synaptics,
...
template void transform_presynaptic_voltage_gpu<float, float2>(const float2* j_ex_presynaptics,
															const float2* j_in_presynaptics,
															const unsigned int n,
															float* j_presynaptics,
															hipStream_t stream);
